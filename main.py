...
def run_training(device):
    ...
    rnn_model = rnn.RNN_model(batch_sz = BATCH_SIZE,vocab_len = len(word_to_int) + 1 ,word_embedding = word_embedding ,embedding_dim= 100, lstm_hidden_dim=128).to(device)
        ...
        for batch in range(n_chunk):
            ...
            for index in range(BATCH_SIZE):
                ...
                if index == 0:
                    ...
                    print('prediction', pre.data.cpu().tolist()) # the following  three line can print the output and the prediction
                    print('b_y       ', y.data.cpu().tolist())   # And you need to take a screenshot and then past is to your homework paper.
                    print('*' * 30)
            loss  = loss  / BATCH_SIZE
            print("epoch  ",epoch,'batch number',batch,"loss is: ", loss.data.cpu().tolist())
            optimizer.zero_grad()
            loss.backward()
...
def gen_poem(begin_word,device):
    ...
    while word != end_token:
        ...
        word = to_word(output.data.cpu().tolist()[-1], vocabularies)
        poem += word
        # print(word)
        # print(poem)
        if len(poem) > 30:
            break
    # 保存诗句到文件
    outfile="poems_result/"+begin_word+'.txt'
...
print(device)
run_training(device)  # 如果不是训练阶段 ，请注销这一行 。 网络训练时间很长。

pretty_print_poem(gen_poem("日",device))
pretty_print_poem(gen_poem("红",device))
pretty_print_poem(gen_poem("山",device))
pretty_print_poem(gen_poem("夜",device))
pretty_print_poem(gen_poem("湖",device))
pretty_print_poem(gen_poem("海",device))
pretty_print_poem(gen_poem("月",device))
pretty_print_poem(gen_poem("君",device))
